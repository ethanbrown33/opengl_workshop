...
int main()
{
	...
	*/
	Shader ourShader("/root/opengl_workshop/src/00_getting_started/03_transformations/03_vertex.glsl", "/root/opengl_workshop/src/00_getting_started/03_transformations/03_fragment.glsl");
	...
	*/
	while (!glfwWindowShouldClose(window))
	{
		...
		glm::mat4 transform = glm::mat4(1.0f); // initialize identity matrix
		transform = glm::translate(transform, glm::vec3(0.5f, -0.5f, 0.0f)); // New image position (0.5, -0.5)
		transform = glm::rotate(transform, (float)glfwGetTime(), glm::vec3(0.5f, 0.5f, 0.5f)); // New image rotation (uses time in radians)

		// Activate shader with transform matrix
		ourShader.use();
